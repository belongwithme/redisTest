...
CountDownLatch是Java并发包(java.util.concurrent)中提供的一个同步工具类，它允许一个或多个线程等待其他线程完成操作。CountDownLatch维护一个计数器，初始值由构造函数设定。每调用一次countDown()方法，计数器减1；当计数器值为0时，所有因调用await()方法而被阻塞的线程都会被释放，继续执行后续操作。
...
当线程调用await()等待时，本质上是尝试获取一个共享锁。获取的条件非常简单：state等于0。如果此时state不为0，线程就会被放入等待队列并挂起。这里的等待队列是一个基于CLH锁实现的FIFO队列，确保了线程唤醒的公平性。
