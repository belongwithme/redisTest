...
```mermaid
graph LR
    subgraph zskiplist["zskiplist 结构"]
        direction TB
        sl_header["*header"] --> H
        sl_tail["*tail"] --> N80
        sl_len["length = 4"]
    ...
    subgraph SkipList["跳表示例 (zskiplistNode)"]
        direction TB

        %% Level 2 (最高索引层)
        subgraph Level_2
            H2(H) --> |"forward (span=3)"| N50_2(50)
            N50_2 --> T2(Null)
        end

        %% Level 1 (中间索引层)
        subgraph Level_1
            H1(H) --> |"forward (span=1)"| N10_1(10)
            N10_1 --> |"forward (span=2)"| N50_1(50)
            N50_1 --> T1(Null)
        end

        %% Level 0 (完整有序双向链表)
        subgraph Level_0
            H(H) -- "forward (span=1)" --> N10("Node<br>score: 10<br>obj: 'A'")
            N10 -- "forward (span=1)" --> N25("Node<br>score: 25<br>obj: 'B'")
            N25 -- "forward (span=1)" --> N50("Node<br>score: 50<br>obj: 'C'")
            N50 -- "forward (span=1)" --> N80("Node<br>score: 80<br>obj: 'D'")
            N80 -- "forward" --> T(Null)
    ...
    %% 节点对齐 (通过不可见链接)
    linkStyle 8,9,10,11,12,13,14,15,16 stroke-width:0px
    ...
    %% 样式定义
    classDef header fill:#d4edda,stroke:#155724,stroke-width:2px;
    classDef node fill:#cce5ff,stroke:#004085,stroke-width:2px;
    classDef struct fill:#f8d7da,stroke:#721c24,stroke-width:2px;

    class H,H1,H2 header;
    class N10,N25,N50,N80,N10_1,N50_1,N50_2 node;
    class T,T1,T2 node;
    class zskiplist,sl_header,sl_tail,sl_len,sl_level struct;
```
...
上层索引主要用于加速查找过程，查找总是从头节点开始向后遍历；

而范围查询和反向遍历只需要在底层进行。

这种设计在保持跳表功能完整的同时，减少了内存开销和实现复杂度
